...
public final class BeepManager implements MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener, Closeable {
	...
	synchronized void updatePrefs() {
		...
		vibrate = PreferenceUtils.getBoolean(activity, PreferencesActivity.KEY_VIBRATE, false);
	...
	private static boolean shouldBeep(Context activity) {
		boolean shouldPlayBeep = PreferenceUtils.getBoolean(activity, PreferencesActivity.KEY_PLAY_BEEP, true);
		if (shouldPlayBeep) {
	...
	private MediaPlayer buildMediaPlayer(Context activity) {
		...
		try {
			AssetFileDescriptor file = activity.getAssets().openFd("sound/qrcode_completed.mp3");
			try {
